...
/* Check if an IP->MAC mapping is in the cache. IP is in network byte order. 
...
/* Do these two things:
   ...
   2) Insert this IP to MAC mapping into the cache and mark it valid. */
...
/* You do not need to call these methods--they are already called in the
   starter code. The init call is a constructor, the destroy call is
   a destructor, and a cleanup thread times out cache entries every 15 seconds. */
