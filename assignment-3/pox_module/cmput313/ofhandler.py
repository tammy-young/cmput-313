...
log = core.getLogger()
FLOOD_DELAY = 5
#default location /home/mininet/pa1/IP_CONFIG
...
class OFHandler (EventMixin):
  def __init__ (self, connection, transparent):
    ...
    self.listenTo(core.cmput313_srhandler)
    core.cmput313_ofhandler.raiseEvent(RouterInfo(self.sw_info, self.rtable))
  ...
  def _handle_PacketIn (self, event):
    ...
    pkt = event.parse()
    raw_packet = pkt.raw
    core.cmput313_ofhandler.raiseEvent(SRPacketIn(raw_packet, event.port))
...
class cmput313_ofhandler (EventMixin):
  """
...
def get_ip_setting():
  ...
  with open(RTABLE_FILE, 'r') as f:
    for line in f:
      parts = line.strip().split()
      if len(parts) == 4:
        if parts[3] == 'eth1':
          RTABLE.append( ('%s' % IP_SETTING['server1'], '%s' % IP_SETTING['server1'], parts[2], parts[3]) )
        elif parts[3] == 'eth2':
          RTABLE.append( ('%s' % IP_SETTING['server2'], '%s' % IP_SETTING['server2'], parts[2], parts[3]) )
        elif parts[3] == 'eth3':
          RTABLE.append( ('%s' % IP_SETTING['client'], '%s' % IP_SETTING['client'], parts[2], parts[3]) )
        else:
          log.info("cannot recognize interface: %s" % parts[3])
          sys.exit(2)
      else:
        log.info("rtable is not set properly")
        sys.exit(2)
...
def launch (transparent=False):
  ...
  """    
  core.registerNew(cmput313_ofhandler, str_to_bool(transparent))
