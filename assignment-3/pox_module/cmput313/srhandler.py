...
class SRServerListener(EventMixin):
  ...
  def __init__ (self, address=('127.0.0.1', 8888)):
    port = address[1]
    self.listenTo(core.cmput313_ofhandler)
    self.srclients = []
  ...
  def _handle_packet_msg(self, conn, vns_msg):
    ...
    core.cmput313_srhandler.raiseEvent(SRPacketOut(pkt, out_port))
...
class cmput313_srhandler(EventMixin):
...
def launch (transparent=False):
  ...
  """
  core.registerNew(cmput313_srhandler)
